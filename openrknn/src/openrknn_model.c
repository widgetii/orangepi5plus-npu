                            ...
                            uint64_t version, struct orknn_model *model)
{
    ...
    /* The .rknn task BO stores per-op compile-time task records,
     * replicated 3× for triple-core layouts. We copy everything into
     * model->task_data; patch_regcmd_addresses iterates model->task_count
     * == total_tasks so every cross-core replica's regcmd slice in the
     * weight BO gets patched, matching the vendor. Per-cycle task BO
     * rewrites (previously loaded from /tmp/rknn_dump/sub*_bo_000_*.bin)
     * are gone — the only cycle-to-cycle delta was the kernel-owned
     * int_status byte, which the RKNPU driver rewrites at completion.
     * Segment derivation and per-op task counting live in
