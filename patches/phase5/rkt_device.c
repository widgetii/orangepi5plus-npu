               ...
               struct pipe_transfer **out_transfer)
{
   ...
   /* Skip PREP_BO for device-resident BOs (already synced, no NPU writes pending)
    * and for cpu_write_only BOs (no need to invalidate cache for write-only access) */
   ...
   /* Use persistent map if available */
   uint8_t *map;
                 ...
                 struct pipe_transfer *transfer)
{
    ...
    * Keep FINI_BO for cpu_write_only BOs — must flush cache to device. */
   ...
   /* Keep persistent map - do not munmap */
                    ...
                    const struct pipe_resource *templat)
{
   ...
   /* Try to find a pooled BO of matching size */
   pthread_mutex_lock(&screen->pool_mutex);
   int best_idx = -1;
   uint64_t best_size = UINT64_MAX;
   util_dynarray_foreach(&screen->bo_pool, struct rkt_bo_pool_entry, entry) {
      int idx = entry - (struct rkt_bo_pool_entry *)util_dynarray_begin(&screen->bo_pool);
      /* Exact size match or up to 2x larger (avoid waste) */
      if (entry->size >= templat->width0 && entry->size <= (uint64_t)templat->width0 * 2) {
   ...
   if (best_idx >= 0) {
      ...
      (void)util_dynarray_pop_ptr(&screen->bo_pool, struct rkt_bo_pool_entry);
   ...
   /* No suitable pooled BO - create new one */
                     ...
                     struct pipe_resource *prsc)
{
   ...
   /* Unmap persistent map */
   if (rsc->persistent_map) {
      os_munmap(rsc->persistent_map, rsc->bo_size);
      rsc->persistent_map = NULL;
   }

   /* Return BO to pool instead of closing (cap pool at 256 entries) */
   pthread_mutex_lock(&screen->pool_mutex);
   unsigned pool_size = util_dynarray_num_elements(&screen->bo_pool,
